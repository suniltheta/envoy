...
getGoogleGrpcChannelCredentials(const envoy::config::core::v3::GrpcService& grpc_service,
                                Api::Api& api, Server::Configuration::ServerFactoryContext& server_context) {
  ...
  return credentials_factory->getChannelCredentials(grpc_service, api, server_context);
}
...
GoogleGrpcUtils::createChannel(const envoy::config::core::v3::GrpcService& config, Api::Api& api, Server::Configuration::ServerFactoryContext& server_context) {
  std::shared_ptr<grpc::ChannelCredentials> creds = getGoogleGrpcChannelCredentials(config, api, server_context);
