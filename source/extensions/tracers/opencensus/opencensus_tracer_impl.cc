...
Driver::Driver(const envoy::config::trace::v3::OpenCensusConfig& oc_config,
    ...
    : oc_config_(oc_config), local_info_(localinfo), server_context_(server_context) {
  ...
  if (oc_config.stackdriver_exporter_enabled()) {
    ...
    if (!oc_config.stackdriver_address().empty()) {
      ...
      auto channel = Envoy::Grpc::GoogleGrpcUtils::createChannel(stackdriver_service, api, server_context_);
  ...
  if (oc_config.ocagent_exporter_enabled()) {
    ...
    if (!oc_config.ocagent_address().empty()) {
      ...
      auto channel = Envoy::Grpc::GoogleGrpcUtils::createChannel(ocagent_service, api, server_context_);
