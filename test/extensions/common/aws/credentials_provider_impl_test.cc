...
public:
  ...
  public:
    ...
    MOCK_METHOD(CredentialsProviderSharedPtr, createWebIdentityCredentialsProvider,
                (Api::Api&, ServerFactoryContextOptRef, Upstream::ClusterManager&,
...
TEST_F(DefaultCredentialsProviderChainTest, NoEnvironmentVars) {
  ...
  EXPECT_CALL(factories_, createInstanceProfileCredentialsProvider(Ref(*api_), _, Ref(cluster_manager_), _, _, _));
  DefaultCredentialsProviderChain chain(*api_, context_, cluster_manager_, "region", DummyFetchMetadata(),
                                        factories_);
}
...
TEST_F(DefaultCredentialsProviderChainTest, RelativeUri) {
  ...
  EXPECT_CALL(factories_, createCredentialsFileCredentialsProvider(Ref(*api_)));
  EXPECT_CALL(factories_, createTaskRoleCredentialsProvider(Ref(*api_), _, Ref(cluster_manager_), _, _, _,
...
TEST_F(DefaultCredentialsProviderChainTest, FullUriWithAuthorizationToken) {
  ...
  EXPECT_CALL(factories_, createTaskRoleCredentialsProvider(
                              Ref(*api_), _, Ref(cluster_manager_), _, _, _, "http://host/path/to/creds", "auth_token"));
  DefaultCredentialsProviderChain chain(*api_, context_, cluster_manager_, "region", DummyFetchMetadata(),
                                        factories_);
}
...
TEST_F(DefaultCredentialsProviderChainTest, NoWebIdentitySessionName) {
  ...
  EXPECT_CALL(factories_,
              createWebIdentityCredentialsProvider(
                  Ref(*api_), _, Ref(cluster_manager_), _, _, _, "/path/to/web_token", "sts.region.amazonaws.com:443",
